...
class Road:
    ...
    def random_lane_position(self):
        """随机选择一个车道并返回一个在该车道上的随机位置"""
        all_lane_items = list(self.network.graph.items())
        if not all_lane_items:
            raise ValueError("No lanes available in the road network.")
        idx = self.np_random.randint(len(all_lane_items))
        lane_index, lane = all_lane_items[idx]
        longitudinal = self.np_random.uniform(5.0, lane.length - 5.0)
        lateral = 0.0
        return np.array(lane.position(longitudinal, lateral))

    def random_lane_heading(self, position):
        """根据坐标查找其所在车道，并返回该位置上的朝向角"""
        lane_index = self.network.closest_lane_index(position)
        lane = self.network.get_lane(lane_index)
        return lane.heading_at(position)
