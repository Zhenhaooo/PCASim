...
from utils.config import pedestrian_statistics_path, pedestrian_pic_save_path


def draw_mutual_scenario(pedestrian_statistics_path, pedestrian_pic_save_path):
    statistics_path_new = f'{pedestrian_statistics_path}/statistics.json'
    with open(statistics_path_new, 'r') as file:
        json_data = json.load(file)
    for at, at_v in json_data['statistics'].items():
        # 遍历一级目录
        for first_level_key, first_level_value in at_v.items():

        # 遍历二级目录
            for second_level_key, second_level_value in first_level_value.items():
                # 记录一级目录 + 二级目录的名字
                col_name = f"{at}->{first_level_key}->{second_level_key}"

                # 如果 'scenario_num' 等于 0，跳过该二级目录
                if second_level_value.get('scenario_num') == 0:
                    continue
                ...
                for key in second_level_value.keys():
                    if key in ['PET', 'pet', 'TTC', 'ttc']:
                        ...
                        axs.bar(hist[1][:-1], norm_counts, width=widths, color='#FF8C00', alpha=1.0, label=f'ego_{at}')
                        axs.set_xlim(left=0)
                        ...
                        save_path = f'{pedestrian_pic_save_path}/mutual'
                        if not os.path.exists(save_path):
                            os.makedirs(save_path)
                        plt.savefig(f'{save_path}/{col_name}.svg', format='svg')


def draw_list_scenario(pedestrian_statistics_path, pedestrian_pic_save_path):
    statistics_path_new = f'{pedestrian_statistics_path}/statistics.json'
    with open(statistics_path_new, 'r') as file:
        json_data = json.load(file)
    for at, at_v in json_data['statistics'].items():
            ...
            for second_level_key, second_level_value in first_level_value.items():
                ...
                for key in second_level_value.keys():
                    if key in ['PET_list', 'TTC_list']:
                        ...
                        plt.tight_layout()
                        plt.grid(True, linestyle='--', alpha=0.6)
                        save_path = f'{pedestrian_pic_save_path}/per'
                        if not os.path.exists(save_path):
                            os.makedirs(save_path)
                        plt.savefig(f'{save_path}/{col_name}.svg', format='svg')
...
if __name__ == '__main__':
    ...
    if args.draw_min:
        draw_mutual_scenario(pedestrian_statistics_path, pedestrian_pic_save_path)
    if args.draw_all:
        draw_list_scenario(pedestrian_statistics_path, pedestrian_pic_save_path)
