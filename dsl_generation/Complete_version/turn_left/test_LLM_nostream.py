...
# === 问题 ===
description_text = '''In this intersection scenario, the ego vehicle executes a left-turn maneuver at a temporarily closed intersection with special lane markings. The scenario features high traffic density (50 vehicles) with two notable adversarial vehicles: 
1) Adversary vehicle 1 approaches from the left side with a significant speed increase of 10.66 m/s
2) Adversary vehicle 2 performs an unsafe right lane change at a distance of 2.12 meters in front of the ego vehicle. '''
# 调用函数获取最相似的描述
...
LLM Input1:{{In this intersection scenario, the ego vehicle executes a left-turn maneuver at a signalized intersection under temporary traffic control measures. The road environment consists of standard lane markings with no special geometry constraints. The scenario features medium traffic density with approximately 20 vehicles in the vicinity. 
...
LLM Input2:{{In this intersection scenario, the ego vehicle executes a left-turn maneuver at a specially designated lane type intersection without temporary traffic modifications. The simulation environment features medium traffic density with approximately 20 vehicles. Two adversarial vehicles create challenging conditions: 
...
{In this intersection scenario, the ego vehicle executes a left-turn maneuver at a temporarily closed intersection with special lane markings. The scenario features high traffic density (50 vehicles) with two notable adversarial vehicles: 
1) Adversary vehicle 1 approaches from the left side with a significant speed increase of 10.66 m/s
2) Adversary vehicle 2 performs an unsafe right lane change at a distance of 2.12 meters in front of the ego vehicle. }
